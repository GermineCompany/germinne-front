...
function CadastroCliente() {
  ...
  const validateNome = (nome) => {
    if (nome.length < 3) {
      setValidateInfos({
        ...validateInfos,
        nome: "Digite um nome com pelo menos 3 caracteres!",
      });
      return false;
    } else {
  ...
  const validateSobrenome = (sobrenome) => {
    if (sobrenome.length < 5) {
      setValidateInfos({
        ...validateInfos,
        sobrenome: "Digite um sobrenome com pelo menos 5 caracteres!",
      });
      return false;
    } else {
  ...
  const validateSenha = (info) => {
    if (infos.senha !== info) {
      setValidateInfos({
        ...validateInfos,
        senha: "As senhas não estão iguais!",
      });
      return false;
    } else {
  ...
  const validateCPF = (cpf) => {
    if (cpf.length !== 11) {
      setValidateInfos({
        ...validateInfos,
        cpf: "Digite um CPF com 11 digitos e sem pontuação!",
      });
      return false;
    } else {
  ...
  const handleRegister = async () => {
    try {
      ...
      localStorage.setItem(
        "userInfo",
        JSON.stringify({ ...result.data, tipo: "cliente" })
      );
    ...
    <div className="box-login-cadastro">
      ...
      <article className="box-inputs-login-cadastro box-inputs-cliente">
        ...
        <div className="box-cadastro box-cadastro-profissional">
          ...
          <button
            className="margin-botao-cadastro-cliente"
